...
/**
 * @author shichunjian
 * @create date 2024-11-02 9:17
 * @Description：该案例演示了通过FlinkSQl实现双流join
 *                                左表emp员工表                         右表dept部门表
 *       内连接        0nCreateAndWrite （创建和写的时候，更新时间）    0nCreateAndWrite （创建和写的时候，更新时间）
 *       左外连接        OnReadAndWrite （读和写的时候，更新时间）      0nCreateAndWrite （创建和写的时候，更新时间）
 *       右外连接      0nCreateAndWrite （创建和写的时候，更新时间）      OnReadAndWrite （读和写的时候，更新时间）
 *       全外连接        OnReadAndWrite （读和写的时候，更新时间）        OnReadAndWrite （读和写的时候，更新时间）
 */
public class Test02_SQL_JOIN {
    public static void main(String[] args) {
        ...
        //1.4 设置状态的保留时间
        tableEnv.getConfig().setIdleStateRetention(Duration.ofSeconds(15));

        //TODO 2.检查点相关的设置(略)
        //TODO 3.从指定的网络端口读取员工数据  并转换为动态表
        SingleOutputStreamOperator<Emp> empDS = env
                .socketTextStream("bigdata.sbx.com", 8888)
        ...
        SingleOutputStreamOperator<Dept> deptDS = env
                .socketTextStream("bigdata.sbx.com", 8889)
