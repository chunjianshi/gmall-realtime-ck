...
/**
 * @author shichunjian
 * @create date 2024-11-23 20:42
 * @Description：独立用户以及回流用户聚合统计
 *
 * 1）启动zk、kafka、HDFS、flume（f1.sh start）、Doris
 * 2）运行DwdBaseLog、DwsUserUserLoginWindow
 */
public class DwsUserUserLoginWindow extends BaseApp {
    public static void main(String[] args) throws Exception{
        new DwsUserUserLoginWindow().start(
                10024,4, Constant.DWS_U_U_L_W,Constant.TOPIC_DWD_TRAFFIC_PAGE
        );
    ...
    public void handle(StreamExecutionEnvironment env, DataStreamSource<String> kafkaStrDS) {
        ...
        //TODO 5.使用Flink的状态编程 判断是否为独立用户及回流用户组
                ...
                new KeyedProcessFunction<String, JSONObject, UserLoginBean>() {
                    ...
                    public void processElement(JSONObject jsonObj, KeyedProcessFunction<String, JSONObject, UserLoginBean>.Context ctx, Collector<UserLoginBean> out) throws Exception {
                        ...
                        if (StringUtils.isNotEmpty(lastLoginDate)) {
                            //若状态中的末次登录日期不为null，进一步判断
                            if (!lastLoginDate.equals(curLoginDate)) {
                                //如果末次登录日期不等于当天日期则独立用户数 uuCt 记为 1 ，并将状态中的末次登陆日期更新为当日，进一步判断。
                                uuCt = 1L;
                                lastLoginDateState.update(curLoginDate);
                                //如果当天日期与末次登录日期之差大于等于8天，则回流用户数backCt置为1。
                            ...
                            //如果状态中的末次登录日期为null，将uuCt置为1，backCt 置为0，并将状态中的末次登录日期更新为当日。
                            uuCt = 1L;
                            lastLoginDateState.update(curLoginDate);
                        }
        ...
        reduceDS.print();
        //TODO 8.将聚合结果写入到Doris
        reduceDS
                .map(new BeanToJsonStrMapFunction<>())
                .sinkTo(FlinkSinkUtil.getDorisSink(Constant.DWS_U_U_L_W));
    }
}
