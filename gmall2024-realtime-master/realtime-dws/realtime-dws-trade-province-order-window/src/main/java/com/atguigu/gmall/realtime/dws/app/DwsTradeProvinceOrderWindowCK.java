...
import com.atguigu.gmall.realtime.common.bean.withProvinceDSBean;
...
/**
 * @author shichunjian
 * @create date 2024-12-18 下午12:06
 * @Description： 省份力度下单聚合统计
 *  * 需要启动的进程：maxwell kafka zk hdfs hbase clickhouse redis 订单明细表的程序DwdTradeOrderDetail DwsTradeProvinceOrderWindowCK
 */
public class DwsTradeProvinceOrderWindowCK  {
    public static void main(String[] args) throws Exception {
        //TODO 1.获取执行环境
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        env.setParallelism(4);
        StreamTableEnvironment tableEnv = StreamTableEnvironment.create(env);
        String topic = Constant.TOPIC_DWD_TRADE_ORDER_DETAIL;
        String groupId = Constant.TOPIC_DWS_T_P_O_W;
        //3.2 创建消费者对象
        ...
        //至此，数据全了，可以写入了！
        //TODO 写入 ClickHouse
        withProvinceDS.print("reduceWithProvinceDS>>>>>>>>>>>");
        withProvinceDS.addSink(JdbcSink.sink(
                "INSERT INTO dws_trade_province_order_window (stt, edt, cur_date, province_id, province_name, order_count, order_amount) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (ps, element) -> {
                    ps.setString(1, element.getStt());
                    ps.setString(2, element.getEdt());
                    ps.setString(3, element.getCurDate());
                    ps.setString(4, element.getProvinceId());
                    ps.setString(5, element.getProvinceName());
                    ps.setString(6, String.valueOf(element.getOrderCount()));
                    ps.setString(7, String.valueOf(element.getOrderAmount()));
                },
